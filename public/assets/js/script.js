...
function artistPage(artist_id, artist_name) {
    ...
    $(document).ready(function () {
        var new_urls = "/pages?artistId= " + artist_id + "";
        $.ajax({
...
function all_songs() {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: "pages?all_songs=" + 10 + "",
            success: function (result) {
                $(".playlist_container").html(result);
...
function all_songs_limit() {
    $(".loading_page").show();
    $(document).ready(function () {
        $.ajax({
            url: "pages?all_songs=" + 50 + "",
            success: function (result) {
                $(".playlist_container").html(result);
                $(".playlist_container").show();
                $(".artist_page").hide();
...
function like_song(user_id) {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: "pages?like_song=" + user_id + "",
            success: function (result) {

                $(".like_songs").html(result);
                $(".playlist_container").hide();
                $(".all_artist_page").hide();
...
function musics_sour(sours) {
    $.ajax({
        url: "pages?list_music=" + sours + "",
        dataType: 'json',
...
function add_like_song(music_id) {
    ...
    $.post("pages", {
        ...
        function () {
            // console.log("add a new like song");
    ...
    if (old_password === '') {
        ...
        if (new_password === '') {
            ...
            if (new_password.length > 6) {
                $(".pass_error").html(" <span class='text_error'> password  lengthe atleast 6  </span>");
            }
            if (password_conf === '') {
                ...
                if (new_password === password_conf) {
                    ...
                    $(".pass_error").html("<span class='text_error'>password confign is not matc</span>");
                }
            }
        }
    }
